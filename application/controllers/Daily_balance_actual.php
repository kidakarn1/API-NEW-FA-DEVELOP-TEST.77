...
class Daily_balance_actual extends CI_Controller
{
	...
	public function GETLOT_TBKKFATHAILAND($lot_dt)
  {
   ...
   return $_YEARS[($Y % 10)].$_MONTH[($M % 12)].$D;
    ...
    public function Get_detail_act($line_cd){
			...
			// $date_now_plus = date("Y-m-d",strtotime("+1 days",strtotime($date_now)))." 08:00:00";
			 $sql_get_detail = "SELECT MAX (id) AS max_id, sum(qty) as total_qty , line_cd, wi_plan, seq_no, item_cd FROM production_actual_detail WHERE st_time >= '$date_now' AND st_time <= '$date_now_plus' AND line_cd = '$line_cd' GROUP BY line_cd, wi_plan, seq_no, item_cd";
			...
			foreach ($get_detail as $key => $value) {
				...
				$get_detail_end_date = $query_detail_dendate->result_array();
			 	...
			 	echo $sql = "select count(id) as c_id from production_emp_detail_realtime where wi_plan = '$wi_plan' and prd_seq_no = '$seq_no'";
				...
				if ($check_data_act == "0"){
					...
					$query_get_sum_act = $this->TBK_FA01->query($sql_get_sum_act);
			 		...
			 		$query_detail_dendate = $this->tbkkfa01_db->query($sql);
